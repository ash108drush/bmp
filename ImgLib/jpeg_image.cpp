...
bool SaveJPEG(const Path& file, const Image& image) {
   ...
   // row_stride = image.GetWidth() * 3; /* JSAMPLEs per row in image_buffer */
    ...
    std::vector<JSAMPLE> jvect;
    jvect.reserve(image.GetWidth() *3);
    ...
    for (int x = 0; x < image.GetWidth(); ++x) {
        jvect.push_back(static_cast<JSAMPLE>(line[x].r));
        jvect.push_back(static_cast<JSAMPLE>(line[x].g));
        jvect.push_back(static_cast<JSAMPLE>(line[x].b));
    }
    jpeg_write_scanlines(&cinfo,reinterpret_cast<JSAMPARRAY> (&jvect), 1);
    jvect.clear();
    }
